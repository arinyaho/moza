...
- **Always activate a profile via the `moza-use` wrapper (or `eval "$(moza use --owner-pid $$ <profile>)"`) — never run `moza use` bare, and never without an owner pid.** The wrapper passes `$$` so the ephemeral files are keyed to your shell; a bare `eval "$(moza use ...)"` keys them to moza's already-exited process, and a stray `moza doctor --gc` then deletes credentials your shell is still using. As an extra safety net `moza use` writes exports to a 0600 ephemeral file and only prints a `source …; rm …` one-liner, so a missed `eval` no longer leaks tokens to stdout. On a real TTY `moza use` refuses outright (use `moza-use` or `eval`).
