...
- **Never run `moza use` bare** — always `eval` it (or use the `moza-use` wrapper). As an extra safety net `moza use` writes exports to a 0600 ephemeral file and only prints a `source …; rm …` one-liner, so a missed `eval` no longer leaks tokens to stdout. On a real TTY `moza use` refuses outright (use `moza-use` or `eval`).
- **In a persistent human shell, pass an owner pid** — use the `moza-use` wrapper (it passes `$$`), or `eval "$(moza use --owner-pid $$ <profile>)"`. Without it the ephemeral files are keyed to moza's already-exited process, and a stray `moza doctor --gc` then deletes credentials the shell is still using. This does **not** apply to the single-call agent form above: that shell is short-lived and dies with the invocation, so keying the files to it or to moza amounts to the same thing, and they are meant to be reclaimed.
